...
class LikeDocumentation extends ControllerTestConfig {
    ...
    void myReceivedLikeList() throws Exception {
        ...
        when(likeFacade.getLikedPeopleList(anyString())).thenReturn(
            List.of(
                LikeReceiveResponse.of(1, "행복", "무엇을 할때 행복한가요?", "user-uuid-1", "유저1", "profile-url", 24, "주소", LocalDateTime.now()),
                LikeReceiveResponse.of(2, "취미", "침대에 누워있고 싶지 않으세요?", "user-uuid-2", "유저2", "profile-url", 32, "주소", LocalDateTime.now()),
                LikeReceiveResponse.of(3, "평화", "돈많은 백수가 되고싶네요", "user-uuid-3","유저3", "profile-url", 64, "주소", LocalDateTime.now()),
                LikeReceiveResponse.of(2, "취미", "침대에 누워있고 싶지 않으세요?", "user-uuid-4", "유저4", "profile-url", 27, "주소", LocalDateTime.now())
            ));
