...
public class UserAcceptanceStep {
    ...
    public static String 신규유저_생성_요청_후_토큰추출(final String username, final String phoneNumber) {
        return 신규유저_생성_요청(username, phoneNumber).jsonPath().getString("accessToken");
    }

    public static ExtractableResponse<Response> 신규유저_생성_요청(final String username, final String phoneNumber) {
        return RestAssured.given().log().all()
            .contentType(MediaType.APPLICATION_JSON_VALUE)
            .body(createUserJoinRequest(username, phoneNumber))
            .when().post("/users/join/signup")
            .then().log().all().extract();
    }

    private static Map<String, Object> createUserJoinRequest(final String username, final String phoneNumber) {
    ...
    public static ExtractableResponse<Response> 그날의_대화토픽_선택_요청(final long fallingIdx,
        final String deleteUser_accessToken) {
        return RestAssured.given().log().all()
            .auth().oauth2(deleteUser_accessToken)
            .contentType(MediaType.APPLICATION_JSON_VALUE)
